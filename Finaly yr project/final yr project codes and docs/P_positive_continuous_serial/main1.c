...
unsigned char initial_message[9] = {'S','e','r','i','a','l',0x0A,0x0D}, cnt=0, angle[10]={0}, enter_angle[] = {"Enter angle as - theta1@ : "};
...
void check_ISR(void)
{
	if(INTCONbits.INT0IF==1)	//external interrupt high priority at RB0 - count for the optical f/b
	{
        ...
        ds = 0.8*error;				//ds= k*error and keep k=0.65 normally		  
  		ds_app = ds;  
		INTCONbits.INT0IF = 0;
  		INTCONbits.INT0IE = 1;
	}
