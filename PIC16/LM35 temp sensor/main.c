...
void main(void)
{
	...
	float g,q,r,p=0,f,temperature,nettemp;
	...
	while(1)
	{
		...
		nettemp=r/100; // by 100 as for loop is running 100 times
		temperature=nettemp*100;

		
		
        /*LM35 gives 0.25 volts at 25 degree celsius
 		so at p we have temperature as 25*p/0.25=100*p */


		LCDClear();
		LCDRow1();
		LCDWriteFloat(nettemp);
		LCDRow2();
		LCDWriteFloat(temperature);
		Delay(2000);
